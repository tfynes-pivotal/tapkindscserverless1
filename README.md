...
```
tanzu apps workload create tapkindscserverless1 -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/tapkindscserverless1-source \
  --type web
```
...
```
tanzu apps workload get tapkindscserverless1
```

You can assign the URL to a variable to make it easier to reference it:

```
URL=$(kubectl get services.serving.knative.dev/tapkindscserverless1 -o=jsonpath='{.status.url}')
 ```
